...
from .forms import UserProfileEditForm, DriverProfileEditForm
...
def into_ride(request, joining_ride_id):
    ...
    sharing_ride.save()
    sharer.delete()
    registeredSharer = RegisteredSharer(registered_sharer_id=request.user, rideid=sharing_ride)
...
def taken_ride_detail(request, take_ride_id):
    taken_ride = get_object_or_404(Ride, pk=take_ride_id)
    owner_name = taken_ride.owner.username
    if not taken_ride.share_id:
        sharer = "No sharer yet"
    else:
        sharer = taken_ride.share_id.username
...
def cancel_owner_ride(request, ride_id):
    ...
    if sharers:
        ...
        send_mail(
            'Ride CANCELLED!',
            'Hi {},\n\nYour sharing ride with {} ride to {}'.format(ride.share_id.username, ride.owner.username, ride.dest) + \
                ' has been CANCELLED by the owner {}'.format(ride.owner.username),
            'erss.hwk1.wx50.cz130@gmail.com',
            sharer_emails,
            fail_silently=False,
...
def sharing_ride_detail(request, ride_id):
    sharing_ride_detail = get_object_or_404(Ride, pk=ride_id)
...
def edit_sharing_ride(request, ride_id):
    ride = get_object_or_404(Ride, pk=ride_id)
    if request.method == 'POST':
        form = sharerEdittingForm(request.POST, ride=ride)
        if form.is_valid():
...
def cancel_sharing_ride(request, ride_id):
    ...
    send_mail(
        'Ride CANCELLED!',
        'Hi {},\n\nYour sharing ride with {} ride to {}'.format(ride.share_id.username, ride.owner.username, ride.dest) + \
            ' has been successfully CANCELLED {}'.format(ride.share_id.username),
        'erss.hwk1.wx50.cz130@gmail.com',
        [ride.share_id.email],
        fail_silently=False,
    )        
